...
| Agent | Model | Purpose |
|-------|-------|---------|
| `main` | `inference/gemma4:26b` | General NemoClaw presentation assistant on the sandbox gateway |
| `livetranscript` | `inference/gemma4:26b` | Live transcript ingestion, slide tracking, off-slide detection |
| `audience` | `inference/gemma4:26b` | Live audience engagement analysis from reactions/questions |

Backend probe endpoints exist for the two specialized agents but are not fully wired into the UI flow:
- `POST /agents/livetranscript` with form fields `transcript`, optional `slide_context`, optional `session_id`
- `POST /agents/audience` with form fields `confused`, `interested`, `lost`, optional `questions`, optional `session_id`
