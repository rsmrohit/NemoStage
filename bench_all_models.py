...
BREV = "http://127.0.0.1:11437"

MODELS = [
    (BREV, "gemma3:1b"),
    (BREV, "nemotron-mini:4b"),
    (BREV, "nemotron-3-nano:4b"),
    (BREV, "gemma3:4b"),
    (BREV, "gemma3:12b"),
    (BREV, "gemma4:26b"),
]
...
TEST_CASES = [
    ("So EEG stands for electroencephalogram, its a method to record nerve signals from the brain using electrodes on the scalp", 2, "current_slide"),
    ("The high-frequency filter lets higher frequency waves pass through while blocking lower frequencies", 4, "current_slide"),
    ("We collected data from five participants over seven days, recording three mental states: focused, unfocused, and drowsed", 10, "current_slide"),
    ("The brainwaves captured by EEG electrodes include alpha, beta, gamma, and theta waves from different brain regions", 3, "current_slide"),
    ("Preprocessing steps include removing outliers, experimenting with different machine learning models", 17, "current_slide"),
    ("EEG uses electrodes on the scalp to capture electrical activity from the brain", 4, "other_slide"),
    ("The notch filter removes line frequency noise, its a special type of band-stop filter", 10, "other_slide"),
    ("We used the FFT algorithm and preprocessing steps to extract features from the different channels", 3, "other_slide"),
    ("The focused state shows much higher frequencies than both the unfocused and drowsed states", 4, "other_slide"),
    ("Lets talk about the ethical implications of using EEG data for commercial brain-computer interfaces", 2, "not_covered"),
    ("The cost of clinical EEG equipment ranges from ten to fifty thousand dollars", 4, "not_covered"),
    ("Deep learning transformers like BERT have been applied to EEG classification tasks recently", 8, "not_covered"),
    ("FDA approval process for medical EEG devices typically takes two to three years", 6, "not_covered"),
]

SLIDE_CONTEXTS = {
    0:  "slide_index=0, title: EEG Signal Analysis\nEEG Signal Analysis - Using Machine Learning to Streamline EEG Readings.",
...
def call_ollama(base_url, model, prompt, timeout=90):
...
summary = []
for base_url, model_name in MODELS:
    print(f"\n{'='*60}", flush=True)
    ...
    for transcript, current_slide, expected in TEST_CASES:
        ...
        prompt = PROMPT.format(current_slide=current_slide, vector_context=vector_context, transcript=transcript)
        try:
            reply, elapsed = call_ollama(base_url, model_name, prompt)
    ...
    print(f"\n  ACCURACY: {correct}/{total} ({acc}%)", flush=True)
    print(f"  LATENCY:  avg={avg_lat:.1f}s  p50={p50:.1f}s", flush=True)
    for label, (c, t) in label_stats.items():
        print(f"    {label}: {c}/{t}", flush=True)
    summary.append((model_name, correct, total, avg_lat))

print(f"\n{'='*60}")
print("SUMMARY (smallest to largest)")
print(f"{'='*60}")
print(f"{'model':<25} {'acc':>10} {'avg_lat':>9}")
for model_name, correct, total, avg_lat in summary:
    flag = " <-- 100%" if correct == total else ""
    print(f"{model_name:<25} {correct}/{total} ({100*correct//total:3d}%)  {avg_lat:>6.1f}s{flag}")
